...
import { FlaskConical, Play, BarChart3, RefreshCw, Download } from "lucide-react";
import type { TestResult } from "../types";

export function ModelTestingDashboard({ results }: { results: TestResult[] }) {
  return (
    <>
                ...
                <div key={result.model} className="rounded-lg border p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant={result.status === "Passed" ? "default" : result.status === "Warning" ? "secondary" : "outline"} className="text-[10px]">{result.status}</Badge>
                    </div>
